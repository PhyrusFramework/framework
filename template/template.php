 ...
 */
function view(string $file, $parameters = [], $outputString = false) {
    ...
    $ret = include(Template::process($file));
...
class Template {
    ...
    public static function generate($original_file) : string {
        ...
        $cached = Cacher::getPath($file);

        if (self::fileExpired($cached)) {
            Cacher::write($cached, $code);
        }
    ...
    public static function has(string $filename) {
        ...
        $cached = Cacher::getPath($tmp);

        $expired = self::fileExpired($cached);
    ...
    public static function isTmp(string $filename) : bool {
        $cached = Cacher::getPath($tmp);
        return strpos($cached, '.' . self::$extension) ? true : false;
    }
